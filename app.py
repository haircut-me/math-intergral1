...
if menu == "📁 시스템 연결 상태":
    ...
    if is_pdf_broken:
        ...
        with t_col2:
            ...
            """.replace("__FILE_PAGE__", str(file_page)).replace("__INIT_RUNNING__", init_running)
        ...
        <div style="background-color: #F8F9FA; padding: 10px; border-radius: 8px; border: 1px solid #E0E0E0; font-family: sans-serif;">
            ...
            <div id="canvas-container" style="width: 100%; height: 350px; overflow-y: auto; overflow-x: hidden; border: 1px solid #D3D3D3; border-radius: 4px;">
        ...
        <script>
            ...
            function resizeCanvas() {
                ...
                canvas.width = canvasContainer.offsetWidth; canvas.height = 800;
                ctx.drawImage(temp, 0, 0); applyModeSettings();
            }
            function applyModeSettings() {
                ctx.lineCap = 'round'; ctx.lineJoin = 'round';
                document.getElementById('btnPen').style.backgroundColor = '#6C757D';
                document.getElementById('btnEraser').style.backgroundColor = '#6C757D';
                document.getElementById('btnScroll').style.backgroundColor = '#6C757D';
                if (currentMode === 'pen') {
                    ctx.globalCompositeOperation = 'source-over'; ctx.strokeStyle = '#1E1E1E'; ctx.lineWidth = 3;
                    document.getElementById('btnPen').style.backgroundColor = '#007BFF';
                    document.getElementById('modeStatus').textContent = '[현재: 연필 쓰기 모드]';
                } else if (currentMode === 'eraser') {
                    ctx.globalCompositeOperation = 'destination-out'; ctx.lineWidth = 24;
                    document.getElementById('btnEraser').style.backgroundColor = '#E0A800';
                    document.getElementById('modeStatus').textContent = '[현재: 부분 지우개 모드]';
                } else if (currentMode === 'scroll') {
                    document.getElementById('btnScroll').style.backgroundColor = '#28A745';
                    document.getElementById('modeStatus').textContent = '[현재: 🖐️ 이동 모드]';
                }
            }
            function setMode(m) { currentMode = m; applyModeSettings(); }
            window.addEventListener('resize', resizeCanvas); setTimeout(resizeCanvas, 200);
            function getPos(e) {
                const r = canvas.getBoundingClientRect();
                return { x: (e.touches ? e.touches[0].clientX : e.clientX) - r.left, y: (e.touches ? e.touches[0].clientY : e.clientY) - r.top };
            }
            canvas.addEventListener('mousedown', (e) => { if(currentMode==='scroll')return; isDrawing=true; const p=getPos(e); lastX=p.x; lastY=p.y; });
            canvas.addEventListener('mousemove', (e) => { if(!isDrawing||currentMode==='scroll')return; const p=getPos(e); ctx.beginPath(); ctx.moveTo(lastX,lastY); ctx.lineTo(p.x,p.y); ctx.stroke(); lastX=p.x; lastY=p.y; });
            window.addEventListener('mouseup', () => isDrawing=false);
            canvas.addEventListener('touchstart', (e) => { if(currentMode==='scroll'||e.touches.length!==1)return; e.preventDefault(); isDrawing=true; const p=getPos(e); lastX=p.x; lastY=p.y; }, {passive:false});
            canvas.addEventListener('touchmove', (e) => { if(!isDrawing||currentMode==='scroll'||e.touches.length!==1)return; e.preventDefault(); const p=getPos(e); ctx.beginPath(); ctx.moveTo(lastX,lastY); ctx.lineTo(p.x,p.y); ctx.stroke(); lastX=p.x; lastY=p.y; }, {passive:false});
            canvas.addEventListener('touchend', () => isDrawing=false);
            function clearCanvas() { const p=ctx.globalCompositeOperation; ctx.globalCompositeOperation='destination-out'; ctx.fillRect(0,0,canvas.width,canvas.height); ctx.globalCompositeOperation=p; }
        </script>
        """
        ...
        <div style="background-color: #EBF3FF; padding: 10px; border-radius: 8px; border: 1px solid #A3C7FF; font-family: sans-serif;">
            ...
            <canvas id="ansCanvas" style="background-color: #FFFFFF; border: 2px dashed #7FB3FF; border-radius: 4px; touch-action: pan-y; width: 100%; height: 110px;"></canvas>
        </div>
        <script>
            const aCanvas = document.getElementById('ansCanvas'); const aCtx = aCanvas.getContext('2d');
            function resizeAnsCanvas() { aCanvas.width = aCanvas.offsetWidth; aCanvas.height = 110; aCtx.strokeStyle = '#0056B3'; aCtx.lineWidth = 4; aCtx.lineCap = 'round'; aCtx.lineJoin = 'round'; }
            window.addEventListener('resize', resizeAnsCanvas); setTimeout(resizeAnsCanvas, 200);
            let aDrawing = false; let aX = 0; let aY = 0;
            function getAPos(e) { const r = aCanvas.getBoundingClientRect(); return { x: (e.touches ? e.touches[0].clientX : e.clientX) - r.left, y: (e.touches ? e.touches[0].clientY : e.clientY) - r.top }; }
            aCanvas.addEventListener('mousedown', (e) => { aDrawing = true; const p = getAPos(e); aX = p.x; aY = p.y; });
            aCanvas.addEventListener('mousemove', (e) => { if (!aDrawing) return; e.preventDefault(); const p = getAPos(e); aCtx.beginPath(); aCtx.moveTo(aX, aY); aCtx.lineTo(p.x, p.y); aCtx.stroke(); aX = p.x; aY = p.y; });
            window.addEventListener('mouseup', () => aDrawing = false);
            aCanvas.addEventListener('touchstart', (e) => { if(e.touches.length!==1) return; e.preventDefault(); aDrawing = true; const p = getAPos(e); aX = p.x; aY = p.y; }, {passive:false});
            aCanvas.addEventListener('touchmove', (e) => { if (!aDrawing||e.touches.length!==1) return; e.preventDefault(); const p = getAPos(e); aCtx.beginPath(); aCtx.moveTo(aX, aY); aCtx.lineTo(p.x, p.y); aCtx.stroke(); aX = p.x; aY = p.y; }, {passive:false});
        ...
        if st.session_state.show_answer_trigger and has_answer_pdf:
            ...
            try:
                ...
                if 0 <= target_ans_page < len(ans_doc):
                    ...
                    st.image(ans_cropped_bytes, use_container_width=True)
                else:
                    st.error("❌ 설정된 해설지 페이지가 범위를 벗어났습니다.")
                ans_doc.close()
            except Exception as e:
